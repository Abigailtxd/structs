...
int compareBypromedio(const void* a, const void* b){
    return ((struct alumno*)a)->promedio
           - ((struct alumno*)b)->promedio;
}
...
int main(){
	...
	qsort(student, n, sizeof(struct alumno), compareBypromedio);
	int mayor=0;
	int nroestudiante;
	for(int i=0; i<estudiantes;i++){
